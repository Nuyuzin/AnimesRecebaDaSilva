...
async function getMeta(id) {
    ...
    try {
        ...
        const videos = [];
        // Seletor para episódios baseado na estrutura comum desses sites
        $('.list-episodios li, .eplister li, .episodios-lista a, .eplister a').each((i, el) => {
...
async function getStreams(id) {
    ...
    try {
        ...
        // Tenta encontrar em scripts se iframes falharem
        if (streams.length === 0) {
            const scripts = $('script').text();
            const playerMatch = scripts.match(/\"file\":\"([^\"]+)\"/);
            if (playerMatch) {
                streams.push({
                    title: "Direct Player",
                    url: playerMatch[1].replace(/\\/g, '')
                });
            }
        }
